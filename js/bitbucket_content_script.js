...
function attachToRepository() {
  ...
  var pageheadActions = document.getElementsByClassName('page-actions')[0];
  pageheadActions.insertAdjacentHTML('afterbegin', '<a class="tweet_btn tweet_repo aui-button" href="#">Tweet</a>');
...
function collectRepositoryInfo () {
  var repositoryInfo = JSON.parse(document.body.getAttribute('data-current-repo'));
  return {
    title: repositoryInfo.fullslug,
    url: 'https://bitbucket.org/' + repositoryInfo.fullslug
  }
}
...
function attachToPullRequest() {
  if(document.getElementsByClassName('pull-request-tab-content').length > 0) {
    ...
    var pageheadActions = document.getElementsByClassName('gh-header-actions')[0];
    pageheadActions.insertAdjacentHTML('afterbegin', '<a class="tweet_btn tweet_pull_request btn" href="#">Tweet PR</a>');
